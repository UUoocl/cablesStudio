...
This operator allows selecting between two modes to capture and upload Syphon frames:

1. **RGBA Texture (Legacy Shared Memory)**:
   - Swift fetches the texture from Metal, reads the raw RGBA pixel data, and writes it to a high-speed shared file in a RAM disk (`/Volumes/CablesRAMDisk/syphon_in_<port>.raw`).
   - The JS operator reads this file on every frame and uploads the pixels to a WebGL texture using `gl.texImage2D`.
   - *Use Case*: Best compatibility with legacy systems. Higher CPU overhead.

2. **WebRTC Stream (GPU-Accelerated Loopback)**:
   - Swift fetches the Metal texture and encodes it directly on the GPU using macOS **VideoToolbox** (H.264 compression).
   - Compressed H.264 Annex B chunks are sent via WebSocket.
   - The JS operator decodes these chunks using the browser's hardware-accelerated **WebCodecs API** (`VideoDecoder`).
   - The decoded `VideoFrame` is pushed to a local WebRTC `RTCPeerConnection` loopback, generating a native **WebRTC Video Track** (`outWebRtcTrack`).
   - Frames are rendered directly to WebGL using `requestVideoFrameCallback` and hardware-accelerated video-to-texture composition.
   - *Use Case*: Highly optimized. Low CPU usage and latency.
