...
export function create(options) {
    ...
    const wsUrl = `${protocol}//${hostname || window.location.hostname}:${port || window.location.port}${path || '/ws'}`;
...
class SlidesStudioClient {
    ...
    connect() {
        console.log(`[SlidesStudioClient] (v1.0.7-DEBUG) Connecting to ${this.url}...`);
        ...
        this.socket.onopen = () => {
            ...
            // Send identification with a small delay to allow connection to settle
            setTimeout(() => {
                if (this.socket && this.socket.readyState === WebSocket.OPEN) {
                    console.log('[SlidesStudioClient] Sending identification...');
                    if (this.authToken) {
    ...
    subscribe(channelName) {
        ...
        // Only send if open; otherwise onopen will handle it for all channels in this.channels
        if (this.socket && this.socket.readyState === WebSocket.OPEN) {
            this._send({ type: 'subscribe', channel: channelName });
        }
