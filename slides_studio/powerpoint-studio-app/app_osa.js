/* ==========================================================================
   PowerPoint Studio App Controller
   ========================================================================== */
...
function initUI() {
    ...
    // Sync button
    document.getElementById('btn-sync-powerpoint').addEventListener('click', syncPowerPointData);
...
function saveLocalSetting(key, val) {
    localStorage.setItem(`powerpoint_studio_${key}`, val);
}

function getLocalSetting(key, defaultVal) {
    const val = localStorage.getItem(`powerpoint_studio_${key}`);
...
function loadSettings() {
    ...
    document.getElementById('obs-source-current').value = getLocalSetting('obs-source-current', 'PowerPoint Presentation');
    document.getElementById('obs-source-next').value = getLocalSetting('obs-source-next', 'PowerPoint Presenter Notes');
...
async function loadObsCredentials() {
    ...
    const hasManualOverride = localStorage.getItem('powerpoint_studio_obs-ip') !== null;
...
function connectCablesSSE() {
    ...
    const handleSseResponse = (event) => {
        try {
            const parsed = JSON.parse(event.data);
            const data = parsed.data?.body || parsed.data || parsed;

            if (data.error) {
                showToast(`JXA Error: ${data.error}`, 'error');
                return;
            }

            handleSlidesUpdate(data);
        } catch (e) {
            console.error("[SSE] Failed to parse slides response:", e);
        }
    };

    // Listen to standard slides-update and custom PowerPointApi responses
...
async function triggerPowerPointApi(command, params = {}) {
    try {
        const queryParams = new URLSearchParams({ request: command, ...params });
        const response = await fetch(`/api/powerpoint?${queryParams.toString()}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const result = await response.json();
        console.log(`[Cables] PowerPointApi response for ${command}:`, result);
        return result;
    } catch (e) {
        console.error(`[Cables] PowerPointApi request error for ${command}:`, e);
        showToast(`Failed to control PowerPoint: ${command}`, 'error');
    }
}

function syncPowerPointData() {
    showToast('Fetching presentation slides...', 'info');
    triggerPowerPointApi('getslides').then(data => {
        if (data && !data.error) {
            handleSlidesUpdate(data);
        }
    });
...
function initTabulator() {
    slidesTable = new Tabulator("#slides-table", {
        ...
        layout: "fitColumns",
        selectableRows: 1,
        placeholder: "Syncing slides from PowerPoint...",
        columns: [
            { 
                title: "#", 
                field: "Index", 
                ...
                cellClick: function(e, cell) {
                    ...
                    triggerPowerPointApi('goto', { slide: slideIndex });
                }
            },
    ...
    // Enable double-click row to trigger transition in PowerPoint editor/play mode
    slidesTable.on("rowDblClick", (e, row) => {
        const data = row.getData();
        showToast(`Jumping to Slide ${data.Index}...`, 'info');
        triggerPowerPointApi('goto', { slide: data.Index });
    });

    // Listen for inline table edits and save back
    slidesTable.on("cellEdited", (cell) => {
        const rowData = cell.getRow().getData();
        updateMetadataOnPowerPoint(rowData.Index, rowData.Scene, rowData.Id);
    });
}
...
function saveSlideSceneMapping() {
    ...
    updateMetadataOnPowerPoint(data.Index, sceneName, data.Id);
}

async function updateMetadataOnPowerPoint(slideIndex, sceneName, uuid) {
    const payload = {
        slide: slideIndex,
        scene: sceneName,
        uuid: uuid
    ...
    // Run setscene command
    const result = await triggerPowerPointApi('setscene', payload);

    if (result && !result.error) {
        showToast('Metadata update sent to PowerPoint!', 'success');
        // Refresh local slides index immediately
        syncPowerPointData();
    }
}
...
async function setSlideSceneFromOBS() {
    ...
    // Trigger setSlideScene shortcut to update PowerPoint
    await updateMetadataOnPowerPoint(data.Index, sceneName, data.Id);
}
...
function navigateOptimistically(direction) {
    ...
    if (targetRow) {
        ...
        if (direction === 'next') {
            triggerPowerPointApi('next');
        } else {
            triggerPowerPointApi('prev');
        }
    } else {
        if (direction === 'next') {
