...
async function triggerKeynoteApi(command, params = {}) {
    try {
        const queryParams = new URLSearchParams({ request: command, ...params });
        const response = await fetch(`/api/keynote?${queryParams.toString()}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const result = await response.json();
        console.log(`[Cables] KeynoteApi response for ${command}:`, result);
        return result;
    } catch (e) {
        console.error(`[Cables] KeynoteApi request error for ${command}:`, e);
        showToast(`Failed to control Keynote: ${command}`, 'error');
    }
}

function syncKeynoteData() {
    showToast('Fetching presentation slides...', 'info');
    triggerKeynoteApi('getslides').then(data => {
        if (data && !data.error) {
            handleSlidesUpdate(data);
        }
    });
        ...
        columns: [
            { 
                ...
                cellClick: function(e, cell) {
                    ...
                    triggerKeynoteApi('goto', { slide: slideIndex });
                }
            },
...
async function updateMetadataOnKeynote(slideIndex, sceneName, uuid) {
    ...
    const result = await triggerKeynoteApi('setscene', payload);
