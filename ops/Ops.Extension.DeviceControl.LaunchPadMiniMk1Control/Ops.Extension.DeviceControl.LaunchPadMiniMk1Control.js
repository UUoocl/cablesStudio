...
const inResetDevice = op.inTriggerButton("Reset Device");
...
const inScrollSpeed = op.inValueSlider("Scroll Speed", 0.5);
...
const inCharColor = op.inValueSelect("Char Color", ["Yellow", "Red", "Orange", "Green", "Lime", "Dim Red", "Dim Green", "Dim Yellow", "Off"], "Yellow");
...
// State diffing cache to prevent MIDI bandwidth overload (max ~400 messages/sec on Mk1)
const lastSentVelocities = new Map();
...
op.setPortGroup("Controls", [inGridSize, inBrightness, inResetDevice, inClearGrid]);
op.setPortGroup("Scrolling Text", [inScrollText, inTriggerScroll, inScrollSpeed]);
...
function updateConnection() {
    ...
    if (outputDevice) {
        ...
        outConnected.set(true);
        // Clear caching on connection update to push fresh frame
        lastSentVelocities.clear();
...
// Trigger Handlers
inResetDevice.onTriggered = resetDevice;
inClearGrid.onTriggered = clearGrid;

inTriggerScroll.onTriggered = function() {
    scrollText = inScrollText.get() || "";
...
inUpdate.onTriggered = function () {
    ...
    if (!tex || !tex.tex) {
        ...
        if (isScrollingText || hasChar || needsCharRedraw) {
            if (isScrollingText) {
                const now = Date.now();
                const speedVal = inScrollSpeed.get();
                const delay = (1.0 - speedVal) * 400 + 50; // 50ms to 450ms
                if (now - lastScrollTime >= delay) {
                    scrollOffset++;
                    lastScrollTime = now;
                    if (scrollOffset >= 8 * scrollText.length) {
    ...
    // Read back downsampled pixels synchronously
    const size = inGridSize.get() === "8x8 Grid" ? 8 : 9;
    const gl = cgl.gl;
    gl.bindFramebuffer(gl.FRAMEBUFFER, fb.getGlFrameBuffer());
    const pixelData = new Uint8Array(size * size * 4);
    gl.readPixels(0, 0, size, size, gl.RGBA, gl.UNSIGNED_BYTE, pixelData);
...
function getScrollTextGrid() {
    const displayGrid = [];
    for (let y = 0; y < 8; y++) {
        displayGrid[y] = new Array(8).fill(false);
        for (let x = 0; x < 8; x++) {
            const globalX = scrollOffset + x;
            if (globalX >= 0) {
                const charIndex = Math.floor(globalX / 8);
                const charCol = globalX % 8;
                if (charIndex < scrollText.length) {
                    const charVal = scrollText[charIndex];
                    const bytes = font[charVal] || font[charVal.toUpperCase()] || font[charVal.toLowerCase()] || font[' '] || [0,0,0,0,0,0,0,0];
                    displayGrid[y][x] = ((bytes[y] >> charCol) & 1) === 1;
                }
            }
        }
    }
...
function processPixels(pixelData, w, h) {
    ...
    if (isScrollingText) {
        const now = Date.now();
        const speedVal = inScrollSpeed.get();
        const delay = (1.0 - speedVal) * 400 + 50;
        if (now - lastScrollTime >= delay) {
            scrollOffset++;
            lastScrollTime = now;
            if (scrollOffset >= 8 * scrollText.length) {
    ...
    const colObj = charColorMap[colorName] || { r: 3, g: 3 };
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            ...
            // Determine pad note/CC index and status for Mk1
            let status = null;
            let note = null;

            if (w === 8) {
                // 8x8 Grid: row 0 is top, row 7 is bottom
                const row = 7 - y;
                const col = x;
                status = 0x90; // Note On Channel 1
                note = 16 * row + col;
            } else {
                // 9x9 Grid (with Buttons)
                if (x < 8 && y < 8) {
                    const row = 7 - y;
                    const col = x;
                    status = 0x90;
                    ...
                    const row = 7 - y;
                    status = 0x90;
                    note = 16 * row + 8; // Right scene launch button
                } else if (x < 8 && y === 8) {
                    const col = x;
                    status = 0xB0; // Control Change Channel 1
                    note = 104 + col; // Top row CC
                }
            }

            if (status !== null && note !== null) {
                let finalR = rVal;
                let finalG = Math.max(gVal, bVal);

                if (displayGrid) {
                    const isActive = (x < 8 && y < 8) ? displayGrid[7 - y][x] : false;
                    if (isActive) {
                        // Prioritize character color: blend (70% char, 30% texture)
                        const charR = colObj.r * (255 / 3);
                        const charG = colObj.g * (255 / 3);
                        finalR = Math.round(charR * 0.7 + rVal * 0.3);
                        finalG = Math.round(charG * 0.7 + Math.max(gVal, bVal) * 0.3);
                    } else {
                        // Dim background
                        finalR = Math.round(rVal * 0.3);
                        finalG = Math.round(Math.max(gVal, bVal) * 0.3);
                    }
                }

                // Scale to 0-3 brightness levels
                const redBr = Math.min(3, Math.max(0, Math.round((finalR / 255) * 3 * brightness)));
                const greenBr = Math.min(3, Math.max(0, Math.round((finalG / 255) * 3 * brightness)));

                // Calculate velocity: (16 * Green) + Red + Flags
                // Flags = 12 (0x0C) for normal write to both buffers
                const velocity = (16 * greenBr) + redBr + 12;

                const cacheKey = (status << 8) | note;
                if (lastSentVelocities.get(cacheKey) !== velocity) {
                    specs.push(status, note, velocity);
                    lastSentVelocities.set(cacheKey, velocity);
                }
            }
        }
    }
    ...
    if (specs.length > 0) {
        sendMidiMessages(specs);
    }
}

function sendMidiMessages(bytes) {
    outMidiMessages.set(bytes);
    if (outputDevice) {
        try {
            for (let i = 0; i < bytes.length; i += 3) {
                const msg = bytes.slice(i, i + 3);
...
function resetDevice() {
    lastSentVelocities.clear();
    sendMidiMessages([0xB0, 0x00, 0x00]);
}
...
function startAnimationLoop() {
    ...
    function animStep() {
        ...
        if (isScrollingText || hasChar || needsCharRedraw) {
            ...
            } else if (isTransitioning || needsCharRedraw) {
                const now = Date.now();
                const duration = inCharTransitionDuration.get() * 1000;
                let t = duration > 0 ? (now - transitionStartTime) / duration : 1.0;
                if (t >= 1.0) {
            ...
            if (isScrollingText || isTransitioning || needsCharRedraw) {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            if (status !== null && note !== null) {
                ...
                if (isActive) {
                    redBr = Math.min(3, Math.max(0, Math.round(colObj.r * brightness)));
                    greenBr = Math.min(3, Math.max(0, Math.round(colObj.g * brightness)));
                }
