// Ops.WebAudio.StrudelInstance.js
...
function getLibraryPath() {
  const relPath = "ops/" + op.objName + "/index.mjs";
...
async function initStrudel() {
  ...
  if (!StrudelLib) {
    ...
    let importPath = libPath;
    if (importPath.startsWith("/") && !importPath.startsWith("file://")) {
      importPath = "file://" + importPath;
    }
    try {
      // Dynamic import of the index.mjs ES Module using absolute path
      StrudelLib = await import(importPath);
    } catch (e) {
      op.setUiError("import_error", "Failed to load index.mjs from " + importPath + ". Error: " + e.message);
      return;
    }
  }
