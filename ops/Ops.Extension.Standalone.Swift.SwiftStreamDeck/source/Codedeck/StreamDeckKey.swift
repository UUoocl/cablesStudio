...
public class StreamDeckKey {
    ...
    public func setColor(red: Int, green: Int, blue: Int) throws {
        ...
        if streamDeck.product.isVersionTwo {
        ...
        } else if streamDeck.product.keyCount == 6 {
    ...
    public func setImage(withActions actions: (CGContext, CGSize) -> Void) {
        ...
        if streamDeck.product.isVersionTwo {
            ...
            // Convert RGBA to BGR
            var bgrData = Data()
            bgrData.reserveCapacity(80 * 80 * 3)
            for i in stride(from: 0, to: imageData.count, by: 4) {
                if i + 2 < imageData.count {
                    bgrData.append(imageData[i + 2]) // B
                    bgrData.append(imageData[i + 1]) // G
                    bgrData.append(imageData[i])     // R
                }
            }
            ...
            let firstPageImageData = Data(bgrData[0 ..< pageOnePacketSize])
            ...
            while i < iconBytes {
                count += 1
                let chunkLength = min(pageTwoPacketSize, iconBytes - i)
                let chunkData = Data(bgrData[i ..< i + chunkLength])
                let secondPage = streamDeck.dataPageTwo(keyIndex: keyIndex, bufIndex: count, data: chunkData)
                streamDeck.write(data: secondPage)
                i += pageTwoPacketSize
            }
        } else {
            // Convert RGBA to BGR
            var bgrData = Data()
            bgrData.reserveCapacity(72 * 72 * 3)
            for i in stride(from: 0, to: imageData.count, by: 4) {
                if i + 2 < imageData.count {
                    bgrData.append(imageData[i + 2]) // B
                    bgrData.append(imageData[i + 1]) // G
            ...
            let firstPageImageData = Data(bgrData[0 ..< StreamDeckKey.NUM_FIRST_PAGE_PIXELS * 3])
            let secondPageImageData = Data(bgrData[StreamDeckKey.NUM_FIRST_PAGE_PIXELS * 3 ..< (StreamDeckKey.NUM_FIRST_PAGE_PIXELS + StreamDeckKey.NUM_SECOND_PAGE_PIXELS) * 3])
