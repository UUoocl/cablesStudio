# Swift Contour ShuttlePRO v2 Standalone Operator

This operator natively monitors input from the **Contour ShuttlePRO v2** multimedia controller in a standalone (Electron) environment using macOS's built-in `IOKit/IOHIDManager` API and a WebSocket channel.
...
1. Open Terminal and navigate to the operator directory:
   ```bash
   cd ops/Ops.Extension.Standalone.Swift.SwiftContourShuttle/
   ```
2. Build the package:
   ```bash
...
3. Copy the compiled executable to the destination directory:
   ```bash
   mkdir -p swift_bin
   cp .build/release/SwiftContourShuttle swift_bin/
   ```
...
*   **`Button Index`** (integer): The last button event index (0–14).
