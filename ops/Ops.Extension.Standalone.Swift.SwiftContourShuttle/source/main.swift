...
print("💡 Activating IOHIDManager ShuttlePRO v2 Monitor...")
if monitor.start() {
    // Check if device is connected right away
    let isConnected = monitor.isDeviceConnected()
    let deviceName = isConnected ? "Contour ShuttlePRO v2" : "Contour ShuttlePRO v2 (Not Connected)"
...
defer {
    print("🛑 Terminating IOHIDManager ShuttlePRO v2 Monitor...")
    monitor.stop()
