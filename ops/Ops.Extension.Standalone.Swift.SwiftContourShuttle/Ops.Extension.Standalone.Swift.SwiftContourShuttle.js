/**
 * Ops.Extension.Standalone.Swift.SwiftContourShuttle
 * 
 * Interfacing natively with Contour ShuttlePRO v2 using compiled macOS Swift binary
...
function killProcess() {
    if (cp) {
        op.log("[SwiftContourShuttle] Terminating background Swift process...");
        try {
            cp.kill();
        } catch (e) {}
        cp = null;
...
function launchProcess(port) {
    ...
    let binaryPath = `${op.patch.config.prefixAssetPath}ops/Ops.Extension.Standalone.Swift.SwiftContourShuttle/swift_bin/SwiftContourShuttle`;
    ...
    if (!fs.existsSync(binaryPath)) {
        op.logError("[SwiftContourShuttle] Swift binary not found at: " + binaryPath);
    ...
    try {
        ...
        op.logWarn("[SwiftContourShuttle] Warning setting execute permissions: " + String(e));
    }

    const args = ["--port", String(port)];
    op.log("[SwiftContourShuttle] Spawning sidecar process: " + binaryPath + " " + args.join(" "));
    ...
    try {
        ...
        cp.stdout.on("data", (data) => {
            ...
            if (str) op.log("[SwiftContourShuttle Output] " + str);
        });

        cp.stderr.on("data", (data) => {
            const str = data.toString().trim();
            if (str) op.logWarn("[SwiftContourShuttle Warning] " + str);
        });

        cp.on("error", (err) => {
            op.logError("[SwiftContourShuttle] Process error: " + err.message);
            outStatus.set("Error: " + err.message);
            stopServerAndProcess();
        });

        cp.on("close", (code) => {
            op.log(`[SwiftContourShuttle] Sidecar exited with code ${code}`);
            cp = null;
            outRunning.set(false);
        ...
        op.logError("[SwiftContourShuttle] Failed to spawn: " + String(e));
...
function stopServerAndProcess() {
    ...
    currentWs = null;
    if (wss) {
        op.log("[SwiftContourShuttle] Closing private WebSocket Server...");
        try {
            wss.close();
        } catch (e) {}
        wss = null;
...
function startServerAndProcess() {
    ...
    try {
        ...
        wss.on("listening", () => {
            ...
            op.log("[SwiftContourShuttle] Private WebSocket Server listening on port " + port);
            outStatus.set("Listening...");
            launchProcess(port);
        });

        wss.on("connection", (ws) => {
            op.log("[SwiftContourShuttle] Swift sidecar connected!");
            currentWs = ws;
            ...
            ws.on("close", () => {
                op.log("[SwiftContourShuttle] Swift sidecar disconnected.");
                if (currentWs === ws) {
                    currentWs = null;
            ...
            ws.on("error", (err) => {
                op.logError("[SwiftContourShuttle] Sidecar connection error: " + err.message);
                outStatus.set("Connection Error");
            });
        });

    } catch (e) {
        op.logError("[SwiftContourShuttle] Failed to start private server: " + String(e));
...
function handleTextMessage(str) {
    ...
    try {
        ...
        if (msg.type === "info") {
            ...
            op.logError("[SwiftContourShuttle] Sidecar error: " + msg.message);
        ...
        op.logWarn("[SwiftContourShuttle] Error parsing message: " + e + " | Message: " + str);
    }
}
