/**
 * Ops.Extension.Standalone.SwiftSidecars.UlanziD100H
...
function killProcess() {
    if (cp) {
        op.log("[UlanziD100H] Terminating native Swift Ulanzi controller process...");
        try {
            cp.kill("SIGTERM");
        } catch (e) {}
        cp = null;
...
function startProcess() {
    ...
    let binaryPath = `${op.patch.config.prefixAssetPath}ops/Ops.Extension.Standalone.SwiftSidecars/Ops.Extension.Standalone.SwiftSidecars.UlanziD100H/swift_bin/UlanziController`;
    if (op.patch && typeof op.patch.filePath === "function") {
        binaryPath = op.patch.filePath(binaryPath);
    }

    if (!fs.existsSync(binaryPath)) {
        op.logError("[UlanziD100H] Swift binary not found at: " + binaryPath);
    ...
    try {
        ...
        op.logWarn("[UlanziD100H] Warning setting execute permissions: " + String(e));
    }

    // op.log("[UlanziD100H] Spawning sidecar process: " + binaryPath);
    ...
    try {
        ...
        cp.on("error", (err) => {
            op.logError("[UlanziD100H] Process error: " + err.message);
            outStatus.set("Error: " + err.message);
            stopProcess();
        });

        cp.on("exit", (code, signal) => {
            // op.log("[UlanziD100H] Process exited with code " + code + ", signal " + signal);
        ...
        op.logError("[UlanziD100H] Failed to spawn: " + String(e));
...
inSendHaptic.onTriggered = () => {
    if (!cp) {
        op.logWarn("[UlanziD100H] Process is not running.");
        return;
    }
    ...
    if (val >= 0 && val <= 100) {
        try {
            ...
            // op.log("[UlanziD100H] Sent haptic strength command: haptic:" + val);
        } catch (e) {
            op.logWarn("[UlanziD100H] Failed to write haptic command to stdin: " + String(e));
        }
    } else {
        op.logWarn("[UlanziD100H] Haptic strength must be between 0 and 100.");
    }
};
