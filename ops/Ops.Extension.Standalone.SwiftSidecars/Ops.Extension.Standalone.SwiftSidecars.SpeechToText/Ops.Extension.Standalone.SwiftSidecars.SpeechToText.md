...
1. **Private Local Pipe**: The JS operator spins up a private WebSocket server on a dynamically allocated local port and spawns the `CablesSpeechToText` background daemon with `--port <port>` arguments.
...
```bash
cd ops/Ops.Extension.Standalone.SwiftSidecars/Ops.Extension.Standalone.SwiftSidecars.SpeechToText
