/**
 * Ops.Team.CablesStudio.WebHid.ExternalContourShuttleXpress
 * 
 * Interfacing with Contour ShuttleXpress visual controllers using WebHID inside a popup window.
 */
...
<head>
    <meta charset="utf-8">
    <title>ShuttleXpress WebHID Bridge</title>
    <style>
        body {
...
<body>
    <h2>ShuttleXpress WebHID Bridge</h2>
    <div class="info">This popup window handles WebHID API device permissions for Cables Studio. Keep open while using Contour ShuttleXpress.</div>
    <div id="status" class="status">Initializing...</div>
    <button id="reqBtn">Request ShuttleXpress</button>
    ...
    <script>
        ...
        function setStatus(text, color = "#aaa") {
            ...
            if (window.opener && window.opener.shuttleXpressBridge_${op.id}) {
                window.opener.shuttleXpressBridge_${op.id}.updateStatus(text);
            }
        }
        ...
        function handleInputReport(event) {
            ...
            const bridge = window.opener && window.opener.shuttleXpressBridge_${op.id};
            if (!bridge) return;
        ...
        async function disconnectDevice() {
            ...
            isConnected = false;
            if (window.opener && window.opener.shuttleXpressBridge_${op.id}) {
                window.opener.shuttleXpressBridge_${op.id}.onDisconnect();
            }
        }
        ...
        async function connectDevice(deviceIndex = 0) {
            ...
            try {
                ...
                isConnected = true;
                const modelName = dev.productName || "ShuttleXpress";
                setStatus("Connected to " + modelName, "#22c55e");
                if (window.opener && window.opener.shuttleXpressBridge_${op.id}) {
                    window.opener.shuttleXpressBridge_${op.id}.onConnect(modelName);
                }
            } catch (e) {
...
function setupOpBridge() {
    window[`shuttleXpressBridge_${op.id}`] = {
...
function openPopupWindow() {
    ...
    childWindow = window.open("about:blank", `shuttlexpress_external_${op.id}`, features);
    if (!childWindow) {
...
op.onDelete = () => {
    clearInterval(windowPollInterval);
    delete window[`shuttleXpressBridge_${op.id}`];
