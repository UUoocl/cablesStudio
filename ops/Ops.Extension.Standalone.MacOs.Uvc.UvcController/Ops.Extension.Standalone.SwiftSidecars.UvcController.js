...
function killProcess() {
    if (cp) {
        op.log("[CablesUvcController] Terminating native Swift UvcController daemon...");
        try {
            cp.kill();
        } catch (e) {}
        cp = null;
...
function stopServerAndProcess() {
    ...
    currentWs = null;
    if (wss) {
        op.log("[CablesUvcController] Closing private WebSocket Server...");
        try {
            wss.close();
        } catch (e) {}
        wss = null;
...
function startServerAndProcess() {
    ...
    try {
        ...
        wss.on("listening", () => {
            ...
            op.log("[CablesUvcController] Private WebSocket Server listening on port " + port);
            launchProcess(port);
        });

        wss.on("connection", (ws) => {
            op.log("[CablesUvcController] Swift sidecar connected!");
            currentWs = ws;
            ...
            ws.on("close", () => {
                op.log("[CablesUvcController] Swift sidecar disconnected.");
                if (currentWs === ws) currentWs = null;
            });

            ws.on("error", (err) => {
                op.logError("[CablesUvcController] Sidecar connection error: " + err.message);
            });
        ...
        op.logError("[CablesUvcController] Failed to start private server: " + String(e));
        outStatus.set("Server Setup Failed");
    }
}

function launchProcess(port) {
    let binaryPath = `${op.patch.config.prefixAssetPath}ops/Ops.Extension.Standalone.SwiftSidecars/Ops.Extension.Standalone.SwiftSidecars.UvcController/swift_bin/CablesUvcController`;
    ...
    if (!fs.existsSync(binaryPath)) {
        op.logError("[CablesUvcController] Swift binary not found at: " + binaryPath);
    ...
    try {
        ...
        op.logWarn("[CablesUvcController] Warning setting execute permissions: " + String(e));
    }
    ...
    op.log("[CablesUvcController] Spawning native sidecar process: " + binaryPath + " " + args.join(" "));
    ...
    try {
        ...
        cp.stdout.on("data", (data) => {
            ...
            if (str) op.log("[CablesUvcController Sidecar] " + str);
        });

        cp.stderr.on("data", (data) => {
            const str = data.toString().trim();
            if (str) op.logWarn("[CablesUvcController Warning] " + str);
        });

        cp.on("error", (err) => {
            op.logError("[CablesUvcController] Process error: " + err.message);
            outStatus.set("Error: " + err.message);
            stopServerAndProcess();
        });

        cp.on("exit", (code, signal) => {
            op.log("[CablesUvcController] Process exited with code " + code + ", signal " + signal);
        ...
        op.logError("[CablesUvcController] Failed to spawn: " + String(e));
...
function sendConfigure() {
    if (currentWs) {
        ...
        try {
            ...
            op.logWarn("[CablesUvcController] Failed to send configuration: " + String(e));
        }
    }
}

// Send command to Swift sidecar
function sendCommand(payload) {
    if (currentWs) {
        try {
            ...
            op.logWarn("[CablesUvcController] Failed to send command: " + String(e));
        }
    }
}
...
function handleTextMessage(str) {
    try {
        ...
        op.logWarn("[CablesUvcController] Error parsing sidecar message: " + String(e));
    }
}
...
inTrigger.onTriggered = () => {
    ...
    try {
        ...
        op.logWarn("[CablesUvcController] Invalid JSON command: " + String(e));
    }
};
