...
Controls Apple Keynote presentation slideshows, advances slides, and updates presenter notes metadata natively using a high-speed background Swift sidecar process.
...
```bash
cd ops/Ops.Extension.Standalone.Swift.SwiftKeynoteApi
swift build -c release
mkdir -p swift_bin
cp .build/release/SwiftKeynoteApi swift_bin/SwiftKeynoteApi
```
