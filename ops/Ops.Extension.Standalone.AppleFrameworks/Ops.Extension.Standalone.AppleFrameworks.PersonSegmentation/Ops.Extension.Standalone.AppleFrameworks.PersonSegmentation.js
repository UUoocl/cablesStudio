...
function initAddon() {
    ...
    let addonPath = "ops/Ops.Extension.Standalone.AppleFrameworks/Ops.Extension.Standalone.AppleFrameworks.PersonSegmentation/person_segmentation.node";
    ...
    if (!fs.existsSync(addonPath)) {
        ...
        op.logError("[PersonSegmentation] Native addon not found at: " + addonPath);
        return false;
    }
    try {
        ...
        op.logError("[PersonSegmentation] Failed to load native addon: " + e.message);
        return false;
    }
}
...
render.onTriggered = () => {
    ...
    try {
        ...
        addon.segment(bufferWrapper, targetW, targetH, quality)
            ...
            .catch(err => {
                isProcessing = false;
                op.logError("[PersonSegmentation] Segmentation background request failed: " + err.message);
                outStatus.set("Processing Error");
            });
            
    } catch (e) {
        isProcessing = false;
        op.logWarn("[PersonSegmentation] Failed to invoke native segment: " + e.message);
    }
};
...
function handleMask(maskBuffer, maskW, maskH) {
    ...
    try {
        ...
        op.logWarn("[PersonSegmentation] Error uploading mask: " + e.message);
    }
}
