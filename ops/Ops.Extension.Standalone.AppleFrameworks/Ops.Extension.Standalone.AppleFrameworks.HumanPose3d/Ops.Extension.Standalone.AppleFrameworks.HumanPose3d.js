...
function initAddon() {
    ...
    let addonPath = "ops/Ops.Extension.Standalone.AppleFrameworks/Ops.Extension.Standalone.AppleFrameworks.HumanPose3d/human_pose3d.node";
    ...
    if (!fs.existsSync(addonPath)) {
        ...
        op.logError("[HumanPose3d] Native addon not found at: " + addonPath);
        return false;
    }
    try {
        ...
        op.logError("[HumanPose3d] Failed to load native addon: " + e.message);
        return false;
    }
}
...
render.onTriggered = () => {
    ...
    // Downsample target calculations (max 384px dimension for high performance 3D pose tracking)
    ...
    try {
        ...
        addon.track(bufferWrapper, targetW, targetH)
            ...
            .catch(err => {
                isProcessing = false;
                op.logError("[HumanPose3d] 3D body pose background tracking failed: " + err.message);
                outStatus.set("Processing Error");
            });
            
    } catch (e) {
        isProcessing = false;
        op.logWarn("[HumanPose3d] Failed to invoke native 3D body pose tracker: " + e.message);
    }
};
