...
function initAddon() {
    ...
    let addonPath = "ops/Ops.Extension.Standalone.AppleFrameworks/Ops.Extension.Standalone.AppleFrameworks.HumanFace/human_face.node";
    ...
    if (!fs.existsSync(addonPath)) {
        ...
        op.logError("[HumanFace] Native addon not found at: " + addonPath);
        return false;
    }
    try {
        ...
        op.logError("[HumanFace] Failed to load native addon: " + e.message);
        return false;
    }
}
...
render.onTriggered = () => {
    ...
    // Downsample target calculations (max 384px dimension for high performance landmark extraction)
    ...
    try {
        ...
        addon.track(bufferWrapper, targetW, targetH)
            ...
            .catch(err => {
                isProcessing = false;
                op.logError("[HumanFace] Face landmarks background tracking failed: " + err.message);
                outStatus.set("Processing Error");
            });
            
    } catch (e) {
        isProcessing = false;
        op.logWarn("[HumanFace] Failed to invoke native face tracker: " + e.message);
    }
};
