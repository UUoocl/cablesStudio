/**
 * Ops.Extension.Standalone.AppleFrameworks.ScreenCaptureKitAudio
 * 
 * High-performance macOS screen/window audio loopback capture using ScreenCaptureKit N-API.
 */
...
function getAddonPath() {
    ...
    const localPath = path.join(dir, "screen_capture_audio.node");
    if (fs.existsSync(localPath)) return localPath;

    let addonPath = "ops/Ops.Extension.Standalone.AppleFrameworks/Ops.Extension.Standalone.AppleFrameworks.ScreenCaptureKitAudio/screen_capture_audio.node";
...
function initAddon() {
    ...
    if (!fs.existsSync(resolvedPath)) {
        op.logError("[ScreenCaptureKitAudio] Native addon binary not found at: " + resolvedPath);
    ...
    try {
        ...
        op.logError("[ScreenCaptureKitAudio] Failed to load native addon: " + String(e));
...
function refreshSources() {
    ...
    addon.getShareableContent((err, content) => {
        if (err || !content) {
            op.logError("[ScreenCaptureKitAudio] Failed to query shareable content: " + String(err));
...
function stopCapture() {
    ...
    if (addon && isCapturing) {
        try {
            ...
            op.logWarn("[ScreenCaptureKitAudio] Error stopping capture: " + e);
        }
    }
    isCapturing = false;
...
function startCapture() {
    ...
    try {
        addon.startCapture({
            ...
            // New frame notification callback.
        });
        ...
        op.logError("[ScreenCaptureKitAudio] Failed to start capture: " + e);
...
function setupAudioContext() {
    ...
    if (!audioCtx) {
        op.logError("[ScreenCaptureKitAudio] AudioContext is not available");
    ...
    if (audioCtx.state === "suspended") {
        audioCtx.resume().catch((re) => {
            op.error("[ScreenCaptureKitAudio] Failed to resume AudioContext:", re);
        });
    }
