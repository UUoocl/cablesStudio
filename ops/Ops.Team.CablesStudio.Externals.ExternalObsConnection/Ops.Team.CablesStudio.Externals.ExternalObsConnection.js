...
<head>
    <meta charset="UTF-8">
    <title>OBS WebSocket Popup Bridge</title>
    <style>
        body {
        ...
        .log-entry.request { color: #38bdf8; }
...
<body>
    <div class="header">
        <div class="title">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="4"></circle></svg>
    ...
    <div class="info-grid">
        <div class="info-item">
            <div class="info-label">OBS Host</div>
            <div id="info-host" class="info-value">-</div>
        </div>
        <div class="info-item">
            <div class="info-label">Broadcast Channel</div>
            <div id="info-channel" class="info-value">-</div>
        </div>
    </div>
    
    <div id="log" class="log-panel">
        <div class="log-entry info">Bridge initialized. Waiting for parent connection parameters...</div>
    </div>
    
    <div class="footer">
        Do not close this window. It routes requests from Cables to OBS.
    </div>

    <script>
        // CHANNEL_NAME_PLACEHOLDER
        
        document.getElementById("info-channel").innerText = CHANNEL_NAME;
        
        const bc = new BroadcastChannel(CHANNEL_NAME);
        
        // Resolve OBSWebSocket from the parent window context (opener)
        const ParentOBSWebSocket = window.OBSWebSocket || (window.opener && window.opener.OBSWebSocket);
        if (!ParentOBSWebSocket) {
            alert("Error: OBSWebSocket constructor could not be resolved from opener window.");
        }
        ...
        function log(msg, type = "info") {
            ...
            bc.postMessage({ type: "log", message: msg });
        }
        ...
        bc.onmessage = async (event) => {
            ...
            if (msg.type === "connect") {
                document.getElementById("info-host").innerText = msg.host + ":" + msg.port;
                log("Received connection request to ws://" + msg.host + ":" + msg.port, "info");
                ...
                try {
                    ...
                    obs.on("Identified", () => {
                        log("Connected and Identified with OBS Studio", "success");
                        updateStatus("connected");
                    });
                    
                    obs.on("ConnectionClosed", (err) => {
                        log("Connection closed: " + (err.reason || "No reason given") + " (Code: " + err.code + ")", "error");
                        updateStatus("disconnected", err.reason || "Connection closed");
                    });
                    
                    obs.on("ConnectionError", (err) => {
                        log("Connection error: " + err.message, "error");
                        updateStatus("disconnected", err.message);
                    });
                    
                    // Intercept and relay dynamic events
                    const originalEmit = obs.emit;
                    obs.emit = function(event, data) {
                        const internalEvents = ["ConnectionOpened", "Hello", "Identified", "ConnectionClosed", "ConnectionError", "__anyEvent"];
                ...
                if (!obs || !obs.identified) {
                    log("Cannot send request: Not connected to OBS", "error");
                    bc.postMessage({
                        type: "response",
                        success: false,
                ...
                if (msg.isBatch) {
                    let requests = [];
                    if (Array.isArray(msg.requestData)) {
                        requests = msg.requestData;
                    } else if (msg.requestData && Array.isArray(msg.requestData.requests)) {
                        requests = msg.requestData.requests;
                    }
                    
                    log("Sending Batch Request (" + requests.length + " items)", "request");
                    try {
                        const results = await obs.callBatch(requests);
                        log("Received batch response success", "success");
                        bc.postMessage({
                            type: "response",
                            success: true,
                        ...
                        log("Batch request failed: " + err.message, "error");
                        bc.postMessage({
                            type: "response",
                            success: false,
                            requestType: msg.requestType || "RequestBatch",
                ...
                if (obs) {
                    log("Disconnecting from OBS...", "info");
                    await obs.disconnect();
                    obs = null;
                }
        ...
        // Notify parent that the bridge is ready to receive credentials
        bc.postMessage({ type: "ready" });
        log("Bridge ready. Broadcasting ready signal...", "info");
    </script>
</body>
</html>`;
    ...
    bc.onmessage = (event) => {
        ...
        switch (msg.type) {
            ...
            case "status":
                ...
                break;
            case "event":
                outEvtName.set(msg.eventName || "");
                outEvtData.set(msg.eventData || null);
                outOnEvent.trigger();
                break;
            case "response":
                ...
                if (msg.success) {
                    ...
                    outError.set(msg.error || "Request failed");
                }
                outOnResponse.trigger();
                break;
            case "log":
                op.log("[OBS Popup] " + msg.message);
                break;
            case "error":
                outError.set(msg.message || "Unknown error in popup");
                break;
        }
    };
}
...
inOpen.onTriggered = () => {
    ...
    const channelName = inChannelName.get() || "obs-external-bridge";
    const features = "width=600,height=550,scrollbars=no,resizable=yes,location=no,toolbar=no,menubar=no,status=no,popup=yes";

    childWindow = window.open("", `obs_external_bridge_${op.id}`, features);
    if (!childWindow) {
        outError.set("Popup window blocked. Please allow popups.");
        outPopupOpen.set(false);
        return;
    }

    // Expose the OBSWebSocket constructor to the child window directly
    childWindow.OBSWebSocket = OBSWebSocket;
    
    // Bind child window WebSocket context to parent window
    window.activeWebSocketConstructor = childWindow.WebSocket;
    ...
    pollInterval = setInterval(() => {
        if (!childWindow || childWindow.closed) {
            ...
            window.activeWebSocketConstructor = null;
            closeBroadcastChannel();
        }
    }, 500);
};
