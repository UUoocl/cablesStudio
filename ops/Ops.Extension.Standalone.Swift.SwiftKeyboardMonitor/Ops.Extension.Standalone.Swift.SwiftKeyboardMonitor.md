...
5. **Advanced Modifiers and Combo Caching**: The daemon captures modifier shifts (`Ctrl`, `Alt`, `Shift`, `Cmd`) dynamically, caching modifier flags thread-safely. When a regular key is pressed, it instantly formats the complete combo string (e.g. `"ctrl + shift + a"`) and streams it directly to the UI operator via the WebSocket server.
