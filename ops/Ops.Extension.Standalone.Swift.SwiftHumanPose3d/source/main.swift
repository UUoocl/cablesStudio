...
final class Pose3DManager: @unchecked Sendable {
    ...
    init(wsClient: WebSocketClient, port: Int) {
        ...
        if fm.fileExists(atPath: ramDiskPath) {
            self.inFilePath = "\(ramDiskPath)/pose3d_in_\(port).raw"
        } else {
            self.inFilePath = "\(NSTemporaryDirectory())pose3d_in_\(port).raw"
        }
    ...
    func processFrame(width: Int, height: Int) {
        ...
        // 3. Perform Apple Vision 3D Human Pose Landmark Request
        let handler = VNImageRequestHandler(cvPixelBuffer: pixelBuffer, options: [:])
        do {
            try handler.perform([request])
        } catch {
            print("❌ Vision 3D pose detection error: \(error.localizedDescription)")
        ...
        for observation in results {
            ...
            // Query all recognized 3D joint landmarks in the pose
            ...
            for (jointName, recognizedPoint) in recognizedPoints {
                // jointName.rawValue.rawValue returns clean string key (e.g. "root", "neck", "left_shoulder")
                let cleanName = jointName.rawValue.rawValue
                
                // Extract 3D position from the joint's transform matrix (simd_float4x4) in column 3 (translation)
                ...
                jointsJSON[cleanName] = [
                    ...
                    "y": y,
                    "z": z,
                    "confidence": 1.0 // 3D joints don't expose individual confidences on VNHumanBodyRecognizedPoint3D struct
                ]
            }
        ...
        // 5. Serialize pose joints and height to JSON and transmit back
...
final class Session: @unchecked Sendable {
    ...
    var pose3dManager: Pose3DManager?
    ...
    func start(host: String, port: Int) {
        ...
        let ws = WebSocketClient(url: serverUrl, onMessage: { [weak self] jsonStr in
            ...
            if type == "frame" {
                ...
                self.pose3dManager?.processFrame(width: width, height: height)
            }
        })
        
        self.wsClient = ws
        self.pose3dManager = Pose3DManager(wsClient: ws, port: port)
        ws.connect()
    }
}
...
print("💡 Activating Swift Human Pose 3D sidecar process, waiting for events...")
