...
// Simple UUID generator for password
...
const 
    ...
    inGenPass = op.inTriggerButton("Generate Password"),
    inLaunch = op.inTriggerButton("Launch OBS"),
    
    outSuccess = op.outBoolNum("Success"),
    outPort = op.outNumber("OBS Port", 4455),
    outPass = op.outString("Password"),
    outError = op.outString("Error");

inWssPort.onChange = () => {
    outPort.set(inWssPort.get());
};
outPort.set(inWssPort.get());

// Password masking is handled in the .json metadata via the "display" property.

// Set initial value if empty
if (!inWssPass.get()) {
    inWssPass.set(generateUUID());
}

// Update output port whenever password changes
inWssPass.onChange = () => {
    outPass.set(inWssPass.get());
};

// Handle regeneration
inGenPass.onTriggered = () => {
    const newPass = generateUUID();
    inWssPass.set(newPass, true); // The second argument 'true' often forces a UI sync in Cables
    outPass.set(newPass);         // Explicitly update output
};

// Ensure output is set on start
outPass.set(inWssPass.get());
