 ...
 * Controls Microsoft PowerPoint presentation slideshows and notes natively using a high-speed Swift sidecar.
 */
...
function killProcess() {
    if (cp) {
        op.log("[SwiftPowerPointApi] Terminating native Swift PowerPoint controller daemon...");
        try {
            cp.kill();
        } catch (e) {}
        cp = null;
...
function stopServerAndProcess() {
    ...
    currentWs = null;
    if (wss) {
        op.log("[SwiftPowerPointApi] Closing private WebSocket Server...");
        try {
            wss.close();
        } catch (e) {}
        wss = null;
...
function startServerAndProcess() {
    ...
    try {
        ...
        wss.on("listening", () => {
            ...
            op.log("[SwiftPowerPointApi] Private WebSocket Server listening on port " + port);
            launchProcess(port);
        });

        wss.on("connection", (ws) => {
            op.log("[SwiftPowerPointApi] Swift sidecar connected!");
            currentWs = ws;
            ...
            ws.on("close", () => {
                op.log("[SwiftPowerPointApi] Swift sidecar disconnected.");
            ...
            ws.on("error", (err) => {
                op.logError("[SwiftPowerPointApi] Sidecar connection error: " + err.message);
            });
        });

    } catch (e) {
        op.logError("[SwiftPowerPointApi] Failed to start private server: " + String(e));
    }
}

function launchProcess(port) {
    let binaryPath = `${op.patch.config.prefixAssetPath}ops/Ops.Extension.Standalone.Swift.SwiftPowerPointApi/swift_bin/SwiftPowerPointApi`;
    ...
    if (!fs.existsSync(binaryPath)) {
        op.logError("[SwiftPowerPointApi] Swift binary not found at: " + binaryPath);
        return;
    }
    ...
    try {
        ...
        op.logWarn("[SwiftPowerPointApi] Warning setting execute permissions: " + String(e));
    }
    ...
    op.log("[SwiftPowerPointApi] Spawning native sidecar process: " + binaryPath + " " + args.join(" "));
    ...
    try {
        ...
        cp.stdout.on("data", (data) => {
            ...
            if (str) op.log("[SwiftPowerPointApi Sidecar] " + str);
        });

        cp.stderr.on("data", (data) => {
            const str = data.toString().trim();
            if (str) op.logWarn("[SwiftPowerPointApi Warning] " + str);
        });

        cp.on("error", (err) => {
            op.logError("[SwiftPowerPointApi] Process error: " + err.message);
            stopServerAndProcess();
        });

        cp.on("exit", (code, signal) => {
            op.log("[SwiftPowerPointApi] Process exited with code " + code + ", signal " + signal);
            cp = null;
        });

    } catch (e) {
        op.logError("[SwiftPowerPointApi] Failed to spawn: " + String(e));
        stopServerAndProcess();
    }
}
...
inTrigger.onTriggered = () => {
    ...
    op.log("[SwiftPowerPointApi] Processing API Request URL:", reqUrl);
    ...
    if (!currentWs) {
        setError("Swift PowerPoint sidecar is not running or connected.", res);
        return;
    }
...
function handleSidecarMessage(str) {
    try {
        ...
        op.logWarn("[SwiftPowerPointApi] Error parsing sidecar message: " + String(e));
    }
}

function setError(msg, res) {
    op.logError("[SwiftPowerPointApi] Error:", msg);
    outErrorMsg.set(msg);
...
function sendSuccessResponse(res, data) {
    if (res && typeof res.end === "function" && !res.headersSent) {
        ...
        op.log("[SwiftPowerPointApi] Sent successful HTTP Response.");
    }
}
...
function sendErrorResponse(res, errorMsg) {
    if (res && typeof res.end === "function" && !res.headersSent) {
        ...
        op.log("[SwiftPowerPointApi] Sent error HTTP Response.");
    }
}
