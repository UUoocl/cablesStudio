...
while i < arguments.count {
    if arguments[i] == "--host" || arguments[i] == "-h", i + 1 < arguments.count {
        ...
        i += 1
    } else if arguments[i] == "--pps", i + 1 < arguments.count {
        if let parsedPps = Int(arguments[i + 1]) {
            pps = parsedPps
        }
        i += 1
    } else if arguments[i] == "--channel" || arguments[i] == "-c", i + 1 < arguments.count {
        channel = arguments[i + 1]
        i += 1
    }
    i += 1
}
...
print("💡 Activating global macOS CGEventTap monitoring for Mouse events at \(pps) PPS...")
monitor.start()

defer {
    print("🛑 Terminating global macOS CGEventTap monitoring...")
    monitor.stop()
