...
async function openPopupWindow() {
  if (op.setUiError) op.setUiError("pip_error", null);
  ...
  const isPipSupported = typeof window !== "undefined" && "documentPictureInPicture" in window && !!window.documentPictureInPicture;

  if (!isPipSupported) {
    if (op.setUiError) {
      op.setUiError("pip_error", "Document Picture-in-Picture API is not supported in this browser. Please use Chrome/Edge v111+.");
    }
    outIsOpen.set(false);
    outWindow.set(null);
    outCanvas.set(null);
  ...
  try {
    ...
    if (op.setUiError) {
      op.setUiError("pip_error", "Failed to open Document Picture-in-Picture window: " + err.message);
    }
    outIsOpen.set(false);
    outWindow.set(null);
    outCanvas.set(null);
  ...
  if (!popupWindow) {
    if (op.setUiError) {
      op.setUiError("pip_error", "Document Picture-in-Picture window could not be opened.");
    }
    outIsOpen.set(false);
    outWindow.set(null);
    outCanvas.set(null);
  ...
  try {
    ...
    popupWindow.document.write(sampleHtmlContent);
    popupWindow.document.close();
    popupWindow.document.title = title;
  } catch (e) {
    op.log("Error writing content to Document PiP window:", e);
  }
