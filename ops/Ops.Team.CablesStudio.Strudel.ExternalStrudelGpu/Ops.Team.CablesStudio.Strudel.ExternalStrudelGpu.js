...
const outImageBitmap = op.outObject("Image Bitmap");
...
if (typeof BroadcastChannel !== "undefined") {
  try {
    bitmapChannel = new BroadcastChannel("strudel_bitmap_channel");
    bitmapChannel.onmessage = (event) => {
      if (event.data && event.data.type === "update-bitmap" && event.data.data) {
        if (currentImageBitmap && typeof currentImageBitmap.close === "function") {
          try { currentImageBitmap.close(); } catch (e) {}
        }
        currentImageBitmap = event.data.data;
        outImageBitmap.set(currentImageBitmap);
        updateGpuTexture(currentImageBitmap);
      }
    };
  } catch (e) {}
}
...
-->
  ...
  <script>
    ...
    if ('BroadcastChannel' in window) {
      ...
      async function sendFrameBitmap() {
        const now = performance.now();
        if (now - lastFrameTime >= 15) {
          lastFrameTime = now;
          if (htmlCanvas && htmlCanvas.width > 0 && htmlCanvas.height > 0 && !isSendingFrame) {
            isSendingFrame = true;
            try {
              const bitmap = await createImageBitmap(htmlCanvas);
              bitmapChannel.postMessage({ type: 'update-bitmap', data: bitmap });
        ...
        requestAnimationFrame(sendFrameBitmap);
      }
      requestAnimationFrame(sendFrameBitmap);
    }
