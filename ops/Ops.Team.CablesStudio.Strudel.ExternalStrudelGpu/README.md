...
- **Flip Y** (`Boolean`, default: `false`): Flips the vertical texture orientation when uploading the `ImageBitmap` to WebGL VRAM.
...
- **Image Bitmap** (`Object`): Outputs the latest 60 FPS `ImageBitmap` object received over `strudel_bitmap_channel` BroadcastChannel.
- **Texture** (`Texture`): Native Cables `CGL.Texture` created from the `ImageBitmap` for direct connection to WebGL materials, shaders, and preview ops.
- **Texture Updated** (`Trigger`): Trigger fired whenever a new frame texture is uploaded to WebGL VRAM.
...
8. **GPU Texture Streaming**: `createImageBitmap()` and BroadcastChannel (`strudel_bitmap_channel`) stream 60 FPS `ImageBitmap` frames to the `Image Bitmap` output port.