...
// Version with ALL landmarks animated for full skeleton coverage
...
// Animate ALL available landmarks
var JOINTS = [
    "NOSE",
    "LEFT_EYE",
    "RIGHT_EYE",
...
export default function (p, op, w, h) {
    ...
    p.onDataChange = function (cablesData) {
        ...
        if (cablesData.landmarks && cablesData.landmarks.length > 0) {
            LIVEMODE = true;
            DATA = [cablesData.landmarks]; // Use the first detected pose
            index = 0;
    ...
    p.setup = function () {
        ...
        for (var i = 0; i < JOINTS.length; ++i) {
            ...
            PAINTLINES.push(new PaintLine(colors[i % colors.length], x, y));
        }
    };
