...
static bool g_prevButtons[15] = {false};
...
static void MatchCallback(void* context, IOReturn result, void* sender, IOHIDDeviceRef device) {
    ...
    std::cout << "[ContourShuttlePro] Shuttle matched." << std::endl;
    SendJSEvent("{\"type\":\"info\",\"status\":\"connected\",\"device\":\"Contour ShuttlePRO v2\"}");
}

static void RemovalCallback(void* context, IOReturn result, void* sender, IOHIDDeviceRef device) {
    g_device_connected = false;
    std::cout << "[ContourShuttlePro] Shuttle removed." << std::endl;
    SendJSEvent("{\"type\":\"info\",\"status\":\"searching\",\"device\":\"Contour ShuttlePRO v2 (Not Connected)\"}");
}
...
napi_value Start(napi_env env, napi_callback_info info) {
    ...
    napi_create_string_utf8(env, "ContourShuttleProCallbackResource", NAPI_AUTO_LENGTH, &resource_name);
